    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">Detalles del Usuario</h2>
                <Link :href="route('admin.usuarios.index')" class="inline-flex items-center gap-1.5 rounded-lg bg-gray-200 dark:bg-gray-700 px-3 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600">
                ...
                <div class="grid gap-6 lg:grid-cols-3">
                    ...
                    <div class="lg:col-span-1">
                        <div class="rounded-lg bg-white dark:bg-gray-800 p-6 shadow">
                            ...
                            <h3 class="text-center text-2xl font-bold text-gray-900 dark:text-white">{{ usuario.name }}</h3>
                            <p class="text-center text-sm text-gray-600 dark:text-gray-400">{{ usuario.email }}</p>
                            ...
                            <div class="mt-6 space-y-4">
                                ...
                                <div class="flex items-center justify-between border-b pb-2">
                                    <span class="text-sm text-gray-600 dark:text-gray-400">Total Pedidos:</span>
                    ...
                    <div class="lg:col-span-2">
                        <div class="rounded-lg bg-white dark:bg-gray-800 p-6 shadow">
                            <h3 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Historial de Pedidos</h3>
                            ...
                            <div v-if="usuario.pedidos && usuario.pedidos.length > 0" class="space-y-4">
                                ...
                                >
                                    <div class="flex items-center justify-between mb-3">
                                        <div>
                                            <p class="text-sm font-medium text-gray-900 dark:text-white">Pedido #{{ pedido.numero_pedido }}</p>
                                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ new Date(pedido.created_at).toLocaleDateString('es-ES') }}</p>
                                        </div>
                                        <span :class="{
                                    ...
                                    <div v-if="pedido.items && pedido.items.length > 0" class="mb-3 space-y-2">
                                        ...
                                        >
                                            ...
                                            <div class="flex-1">
                                                ...
                                                <p class="text-xs text-gray-500 dark:text-gray-400">{{ item.tienda_nombre }}</p>
                                            </div>
                                            <div class="text-right">
                                                <p class="text-xs text-gray-600">{{ item.cantidad }}x {{ Number(item.precio_unitario).toFixed(2) }}€</p>
                                                <p class="font-semibold text-gray-900 dark:text-white">{{ Number(item.subtotal).toFixed(2) }}€</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center justify-between border-t pt-3">
                                        ...
                                        <span class="text-lg font-bold text-gray-900 dark:text-white">{{ Number(pedido.total).toFixed(2) }}€</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-else class="py-12 text-center">
                                ...
                                <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white">Sin pedidos</h3>
                                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Este usuario aún no ha realizado ningún pedido.</p>
                            </div>
                        </div>
                    </div>
                </div>
