    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <nav class="sticky top-0 z-40 border-b border-gray-200 bg-white shadow-sm">
            <div class="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                <div class="flex items-center gap-3">
                    <Link :href="route('profile.edit')" class="text-sm text-gray-500 transition hover:text-gray-900">Mi perfil</Link>
                    <Link href="/" class="flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm text-gray-600 transition-colors hover:bg-gray-100 hover:text-gray-900">
        ...
        <main class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                ...
                <p class="mt-1 text-sm text-gray-500">Consulta el estado de tus pedidos y tu historial de compras</p>
            </div>

            <!-- Tabs -->
            <div class="mb-6 flex gap-1 rounded-xl bg-gray-100 p-1">
                <button @click="tabActivo = 'activos'" :class="['flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition-all duration-200', tabActivo === 'activos' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700']">
                ...
                <button @click="tabActivo = 'historial'" :class="['flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold transition-all duration-200', tabActivo === 'historial' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700']">
                    Historial
                    <span class="text-xs text-gray-400">({{ pedidosHistorial.total }})</span>
                </button>
            </div>
            ...
            <div v-if="tabActivo === 'activos'">
                <div v-if="pedidosActivos.length === 0" class="flex flex-col items-center py-24 text-center">
                    ...
                    <h2 class="text-xl font-bold text-gray-800">No tienes pedidos activos</h2>
                    <p class="mt-2 text-sm text-gray-500">¡Explora las tiendas y realiza tu próximo pedido!</p>
                ...
                <div v-else class="space-y-4">
                    <div v-for="pedido in pedidosActivos" :key="pedido.id" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm transition hover:shadow-md">
                        <button class="flex w-full items-center justify-between gap-4 px-6 py-5 text-left transition-colors hover:bg-gray-50" @click="toggleExpand(pedido.id)">
                            <div class="min-w-0 flex-1">
                                ...
                                <p class="mt-1 text-sm text-gray-500">{{ formatFecha(pedido.created_at) }} · {{ pedido.items_count }} producto{{ pedido.items_count !== 1 ? 's' : '' }}</p>
                            </div>
                            <div class="shrink-0 text-right">
                                <p class="text-lg font-extrabold text-primary-600">{{ Number(pedido.total).toFixed(2) }}€</p>
                            </div>
                            <svg class="h-5 w-5 shrink-0 text-gray-400 transition-transform duration-200" :class="{ 'rotate-180': expandidos.has(pedido.id) }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div v-if="expandidos.has(pedido.id)" class="overflow-hidden border-t border-gray-100">
                                <div class="divide-y divide-gray-100">
                                    <div v-for="item in pedido.items" :key="item.id" class="flex items-center gap-4 px-6 py-3">
                                        ...
                                        <div class="w-16 text-right text-sm font-semibold text-gray-800">{{ Number(item.subtotal).toFixed(2) }}€</div>
                                    </div>
                                </div>
            ...
            <div v-if="tabActivo === 'historial'">
                <div v-if="pedidosHistorial.data.length === 0" class="flex flex-col items-center py-24 text-center">
                    ...
                    <h2 class="text-xl font-bold text-gray-800">Sin historial aún</h2>
                    <p class="mt-2 text-sm text-gray-500">Aquí verás los pedidos entregados y cancelados.</p>
                </div>

                <div v-else class="space-y-4">
                    <div v-for="pedido in pedidosHistorial.data" :key="pedido.id" class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm opacity-90 transition hover:opacity-100 hover:shadow-md">
                        <button class="flex w-full items-center justify-between gap-4 px-6 py-5 text-left transition-colors hover:bg-gray-50" @click="toggleExpand('h-' + pedido.id)">
                            <div class="min-w-0 flex-1">
                                <div class="flex flex-wrap items-center gap-2">
                                    <span class="font-bold text-gray-900">{{ pedido.numero_pedido }}</span>
                                    <span :class="['flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-semibold', getEstado(pedido.estado).bg, getEstado(pedido.estado).text]">
                            ...
                            <div class="shrink-0 text-right">
                                <p class="text-lg font-extrabold text-gray-700">{{ Number(pedido.total).toFixed(2) }}€</p>
                            </div>
                            <svg class="h-5 w-5 shrink-0 text-gray-400 transition-transform duration-200" :class="{ 'rotate-180': expandidos.has('h-' + pedido.id) }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                            </svg>
                        </button>
                        <Transition enter-active-class="transition-all duration-200" enter-from-class="max-h-0 opacity-0" enter-to-class="max-h-screen opacity-100" leave-active-class="transition-all duration-200" leave-from-class="max-h-screen opacity-100" leave-to-class="max-h-0 opacity-0">
                            <div v-if="expandidos.has('h-' + pedido.id)" class="overflow-hidden border-t border-gray-100">
                                <div class="divide-y divide-gray-100">
                                    <div v-for="item in pedido.items" :key="item.id" class="flex items-center gap-4 px-6 py-3">
                                        <img :src="item.producto_imagen || '/images/logo.png'" :alt="item.producto_nombre" class="h-12 w-12 flex-shrink-0 rounded-lg object-cover" />
                                        <div class="min-w-0 flex-1 text-sm">
                ...
                <div v-if="pedidosHistorial.last_page > 1" class="mt-8 flex items-center justify-center gap-2">
                    <Link v-for="link in pedidosHistorial.links" :key="link.label" :href="link.url || '#'" v-html="link.label" :class="['rounded-lg px-3 py-2 text-sm transition-colors', link.active ? 'bg-primary-500 font-bold text-white' : link.url ? 'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200' : 'cursor-not-allowed bg-white text-gray-300 border border-gray-100']" />
                </div>
            </div>
