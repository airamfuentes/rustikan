                ...
                <div v-if="totalResenas > 0 || canReview || user" class="grid gap-8 lg:grid-cols-3">
                    ...
                    <div class="lg:col-span-2 space-y-4">
                        ...
                        <div v-if="canReview && !showReviewForm"
                            ...
                            <p :class="['text-base font-semibold', isDark ? 'text-white' : 'text-gray-900']">{{ t('store.be_first') }}</p>
                            <p :class="['mt-1 text-sm', isDark ? 'text-gray-400' : 'text-gray-500']">{{ t('store.click_to_write') }}</p>
                            <span class="mt-4 inline-flex items-center gap-1.5 rounded-xl bg-primary-500 px-4 py-2 text-sm font-bold text-white shadow">
                        ...
                        <div v-if="totalResenas === 0 && !canReview && !showReviewForm"
                            ...
                            <p v-if="user && user.role === 'user'" :class="['mt-1 text-sm', isDark ? 'text-gray-400' : 'text-gray-500']">{{ t('store.need_order') }}</p>
                        </div>
